...
async def start_nausys_bot(request: BotIntervalRequest = BotIntervalRequest(),
...
async def stop_nausys_bot(current_user: str = Depends(get_current_user)):
    """Stop the Nausys bot"""
    return await bot_controller.stop_bot(BotType.NAUSYS)


@router.get("/nausys/status", response_model=BotStatusResponse)
async def get_nausys_status(current_user: str = Depends(get_current_user)):
...
async def start_mmk_bot(request: BotIntervalRequest = BotIntervalRequest(),
                        current_user: str = Depends(get_current_user)):
    """
...
async def stop_mmk_bot(current_user: str = Depends(get_current_user)):
    """Stop the MMK bot"""
    return await bot_controller.stop_bot(BotType.MMK)


@router.get("/mmk/status", response_model=BotStatusResponse)
async def get_mmk_status(current_user: str = Depends(get_current_user)):
